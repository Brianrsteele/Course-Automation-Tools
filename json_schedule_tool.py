...
def main():
    ...
    # and keys for values
    important_dates = {item: value for (item, value) in semester.items() 
    ...
    for date in meeting_days:
        ...
        if date.strftime('%A') in DAYS_OF_WEEK:
            if date_string in important_dates:
                schedule.append((date_string_with_weekday, important_dates[date_string]))
            else:
...
def return_schedule(start, end):
    ...
    schedule = [start + datetime.timedelta(days=x) for x in range((end - start).days + 1)]
    return schedule
...
def export_md(schedule):
    ...
    """
    document = '# Important Dates for ' + COURSE + '-' + SECTION + ', ' + SEMESTER + ' ' + YEAR + '#\n'
...
def export_docx(schedule):
    ...
    title = important_dates.add_heading('Important Dates for ' + \
                                        COURSE + '-' + SECTION + ', ' + \
                                        SEMESTER + ' ' + YEAR, 0)
    for date in schedule:
